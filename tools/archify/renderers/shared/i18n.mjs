export const SUPPORTED_LOCALES = ['en', 'zh-CN', 'th'];
...
const MESSAGE_PAIRS = {
  ...
  'diagram.description.architecture': ['An architecture diagram generated by Archify.', '由 Archify 生成的架构图。', 'แผนภาพสถาปัตยกรรมสร้างโดย Archify'],
  'diagram.description.workflow': ['A workflow diagram generated by Archify.', '由 Archify 生成的工作流图。', 'แผนภาพเวิร์กโฟลว์สร้างโดย Archify'],
  'diagram.description.sequence': ['A sequence diagram generated by Archify.', '由 Archify 生成的时序图。', 'แผนภาพลำดับการทำงานสร้างโดย Archify'],
  'diagram.description.dataflow': ['A data-flow diagram generated by Archify.', '由 Archify 生成的数据流图。', 'แผนภาพการไหลของข้อมูลสร้างโดย Archify'],
  'diagram.description.lifecycle': ['A lifecycle diagram generated by Archify.', '由 Archify 生成的生命周期图。', 'แผนภาพวงจรชีวิตสร้างโดย Archify'],
  'node.focus': ['Focus {label}', '聚焦{label}', 'โฟกัส {label}'],
  'node.focus.detail': ['Focus {label}, {detail}', '聚焦{label}，{detail}', 'โฟกัส {label}, {detail}'],
  'node.context.architecture': ['Architecture component', '架构组件', 'คอมโพเนนต์สถาปัตยกรรม'],
  'node.context.workflow': ['Workflow node', '工作流节点', 'โหนดเวิร์กโฟลว์'],
  'node.context.sequence': ['Sequence participant', '时序参与者', 'ผู้เข้าร่วมลำดับการทำงาน'],
  'node.context.dataflow': ['Data-flow node', '数据流节点', 'โหนดการไหลของข้อมูล'],
  'node.context.lifecycle': ['Lifecycle state', '生命周期状态', 'สถานะวงจรชีวิต'],
  'legend.title': ['Legend', '图例', 'คำอธิบายสัญลักษณ์'],
...
for (const [key, messages] of Object.entries(MESSAGE_PAIRS)) {
  if (messages.length < 2 || messages.some((message) => typeof message !== 'string')) {
    throw new Error(`Incomplete Archify i18n tuple ${JSON.stringify(key)}`);
  }
}

const CATALOGS = {
  'en': Object.fromEntries(Object.entries(MESSAGE_PAIRS).map(([key, tuple]) => [key, tuple[0]])),
  'zh-CN': Object.fromEntries(Object.entries(MESSAGE_PAIRS).map(([key, tuple]) => [key, tuple[1]])),
  'th': Object.fromEntries(Object.entries(MESSAGE_PAIRS).map(([key, tuple]) => [key, tuple[2] || tuple[0]])),
};
