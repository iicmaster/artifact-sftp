...
#            [--basic-auth -] ( --pass - | --ssh-key PATH | --op-ref op://... )
#   secrets are read from stdin, one per line, password first then basic-auth:
...
for v in "$HOST" "$SUSER" "$PORT" "$REMOTE" "$URL" "$TOOL" "$BASIC" "$SSH_KEY" "$OP_REF"; do
  [ -z "$v" ] || _shell_safe "$v" || die "value contains characters that break config sourcing: '$v'"
done

# Secrets come from stdin, never argv (argv leaks via ps/history). When both --pass - and
# --basic-auth - are used, feed TWO lines: password first, then basic-auth. Validated before
# any side effect so a bad secret fails fast.
PASS=''
if [ "$READ_PASS" = 1 ]; then
